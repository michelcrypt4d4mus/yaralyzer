...
def test_yaralyze(il_tulipano_path, tulips_yara_path):
    ...
    with_pattern_output = _run_with_args(il_tulipano_path, '-r', 'tulip.{1,2500}tulip')
    assert _line_count(with_pattern_output) == _line_count(with_yara_file_output)
...
def _line_count(_string: str) -> int:
